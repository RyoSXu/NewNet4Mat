...
def subprocess_fn(args):
    ...
    args.cfg_params['dos_minmax'] = args.dos_minmax
    args.cfg_params['dos_zscore'] = args.dos_zscore
    args.cfg_params['scale_factor'] = args.scale_factor
    args.cfg_params['apply_log'] = args.apply_log
    ...
    train_dataloader = builder.get_dataloader(
        split='train', smear=args.smear,
        dos_minmax=args.dos_minmax, dos_zscore=args.dos_zscore,
        scale_factor=args.scale_factor, apply_log=args.apply_log
    )
    logger.info('Train dataloaders build complete')
    test_dataloader = builder.get_dataloader(
        split='test', smear=args.smear,
        dos_minmax=args.dos_minmax, dos_zscore=args.dos_zscore,
        scale_factor=args.scale_factor, apply_log=args.apply_log
    )
    logger.info('Test dataloaders build complete')
    valid_dataloader = builder.get_dataloader(
        split='valid', smear=args.smear,
        dos_minmax=args.dos_minmax, dos_zscore=args.dos_zscore,
        scale_factor=args.scale_factor, apply_log=args.apply_log
    )
    ...
    lr_scheduler_params = args.cfg_params['model']['params']['lr_scheduler']
    for key in lr_scheduler_params:
        if lr_scheduler_params[key].get('by_step', False):
            for k in lr_scheduler_params[key]:
                if "epochs" in k:
                    lr_scheduler_params[key][k] *= steps_per_epoch
    ...
    model_without_ddp.trainer(
        train_dataloader, test_dataloader, valid_dataloader,
        builder.get_max_epoch(), checkpoint_savedir=args.run_dir, resume=args.resume
    )
...
def main(args):
    ...
    cfg_params['dataset']['smear'] = args.smear
...
if __name__ == "__main__":
    ...
    parser.add_argument('--per_cpus',     type=int,      default=1,                                  help='Number of per CPUs to use')
    ...
    parser.add_argument('--dos_minmax',   type=str2bool, default=False,                              help='min-max normalization')
    parser.add_argument('--dos_zscore',   type=str2bool, default=False,                              help='z-score normalization')
    parser.add_argument('--scale_factor', type=float,    default=1.0,                                help='scale factor for normalization')
    parser.add_argument('--apply_log',    type=str2bool, default=False,                              help='apply log transformation')
